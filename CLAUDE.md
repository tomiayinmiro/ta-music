...
## Decisions made pre-Phase 7: Feedback & Help (not in the original brief)

- **A small in-app "Feedback & Help" section was added ahead of Phase 7** (approved 2026-09-07) — a nav-drawer entry opening a two-tab screen: Help (9 expandable FAQ entries) and Feedback (bug report / feature request / general comment form posting to Web3Forms). No Stitch design exists for this screen — built from sonic_sanctuary_2 tokens, matching the manual-lyrics-editor's plain `TextField`-with-`errorText` form pattern. See `DESIGN_MAP.md`.
- **Feedback submission goes to Web3Forms** (`https://api.web3forms.com/submit`) via the app's shared `Dio` client, mirroring `TranslationClient`'s shape (`FeedbackClient`, `lib/data/services/feedback/feedback_client.dart`). The access key is embedded as a `static const` — per Web3Forms' own docs this is a public-facing per-form identifier, not a secret. App version is hardcoded (`kAppVersion` in `lib/core/constants/app_info.dart`) as `'1.0.0'` pending a real `package_info_plus`-backed value later.
- **Nav drawer restructured into three fixed/scroll/fixed bands** (approved 2026-09-07, `app_nav_drawer.dart`) — a non-scrolling header, an `Expanded`+`SingleChildScrollView` middle band for Lounge/The Gallery/Favorites/Local Stats, and a non-scrolling bottom band (Feedback & Help, then Settings) that's always fully visible. The drawer previously used a fixed `Column` with a `Spacer()` to pin Settings to the bottom; on a screen short enough that the item list (plus the expanded Local Stats panel) overflowed the drawer's height, that approach silently clipped whatever came last instead of wrapping it in a scroll view — reproduced live on the Tecno BF6 test device, where the newly-added Feedback & Help entry rendered fine on the taller Windows debug window but never appeared on the phone at all. Any further drawer entries should go in the appropriate band, not appended after the old `Spacer()`-based pattern.
- **`flutter install --debug` does not rebuild the app — it only reinstalls whatever artifact already sits in `build/app/outputs/flutter-apk/`.** Confirmed via `-v` output (no `Gradle assembleDebug` task ran) and by comparing file timestamps after a source change: the APK stayed dated from an earlier build across two subsequent `flutter install` calls, so a phone under test kept running stale code with no error or warning. For a real Android debug rebuild, run `flutter build apk --debug` explicitly first (verify by checking the APK's own timestamp advanced), then install — `adb install -r <path>` or `flutter install`, either is fine once the artifact itself is confirmed fresh. Worth checking `adb shell dumpsys package <applicationId>` (`lastUpdateTime`) after installing to confirm a device actually received a new build, rather than trusting the install command's own "Success" output alone.

## Decisions made during Version/About restructuring (not in the original brief)

- **Version and About split out of Settings into their own nav-drawer entries** (approved 2026-09-08) — the "About" section that had briefly lived in Settings (version display + "Check for updates") is removed entirely. The nav drawer's pinned bottom band now holds four entries, always visible, in order: **Version, About, Feedback & Help, Settings**. The Version entry shows a live "Current version X.X.X" subtitle (from `packageInfoProvider`) right in the drawer, without needing to open the screen.
- **New `lib/features/update/screens/version_screen.dart` and `lib/features/about/screens/about_screen.dart`.** Version hosts the "Check for updates" button and Play-Store-vs-direct-install messaging moved verbatim from the old Settings section (behavior unchanged), plus a new "Last checked" timestamp read from `SettingsRepository.getLastUpdateCheckAt()` (that getter already existed but wasn't surfaced in any UI yet). About is static content only (app info, made-by, privacy, open-source acknowledgments, release notes) — no Stitch design exists for either screen; both built from sonic_sanctuary_2 tokens matching Settings/Feedback & Help's existing section patterns, and both use `AppScaffold` (not a bare `Scaffold`), per the Phase 3 mini-player rule.
- **App icon asset added at `assets/images/app_icon.png`**, for the About screen's icon display — copied from the existing Android launcher icon (`mipmap-xxxhdpi/ic_launcher.png`) as a placeholder, since no dedicated Flutter-asset app icon existed anywhere in the repo. Tracked in `BACKLOG.md` as "Replace app icon before release."
- **The `kAppVersion`/`lib/core/constants/app_info.dart` reference in the Feedback & Help section above is now stale** — that file was deleted as part of the (separate, uncommitted) in-app-update-checking work that landed alongside this restructuring. Every version read in the app (Feedback form, Version screen, nav drawer subtitle) now goes through `packageInfoProvider` (`PackageInfo.fromPlatform()`), not a hardcoded constant.

## Decisions made during Settings expansion, Round 1 — theme/gapless/speed (not in the original brief)

- **Light theme ships as a real, persisted setting in Round 1, but `AppTheme.light` is a temporary stub that resolves identically to `AppTheme.dark`** (approved 2026-09-08) — a full inverted sonic_sanctuary_2 light palette is real design work, deferred to v1.5 under BACKLOG.md's "Custom themes / color schemes". `MaterialApp` is fully wired (`theme`/`darkTheme`/`themeMode` from `themeModeProvider`, applied immediately, no restart), so picking Light (or System on a light-mode device) is a real choice that just visually matches Dark until that palette lands.
- **Gapless playback was built, then fully removed the same day (2026-09-09)** — its `_onIndexChanged`-reactive pause/resume approach broke core auto-advance: `just_audio` 0.10.6's `currentIndexStream` has no `.distinct()` (confirmed in its source — `_currentIndexSubject.addStream(sequenceStateStream.map(...))`, re-emitting on every `playbackEventStream` event, not just real index changes), so the gap-pause's own `pause()`/`play()` calls retriggered themselves indefinitely, and a follow-up fix for that regressed into auto-advance simply never resuming after the first track. Rather than keep patching a mechanism built on a stream that doesn't behave the way its design assumed, the whole feature (toggle, `SettingsRepository`/DAO keys, `gaplessPlaybackProvider`, all `AudioPlayerHandler` state — `_gaplessPlayback`, `_manualTransitionPending`, `_lastKnownIndex`, `_gapPauseTimer`, `_pausedForGap`, `_insertGapPause`) was removed, restoring `_onIndexChanged`/`playFromSong`/`restoreState`/`pause`/the three skip methods to their exact pre-Round-1 form. See BACKLOG.md's "Gapless playback" deferred item for the path back in.
- **Playback speed is applied reactively, not through the `PlaybackHandler` interface.** `AudioPlayerHandler` subscribes to `SettingsRepository.watchPlaybackSpeed()` in `_init()` and calls `_player.setSpeed()` directly whenever it changes — the same pattern already used for `resumeAfterInterruption`, rather than adding `setSpeed()`/a speed stream to the interface and its test fake. Settings and the Now Playing quick-access indicator both write straight to `SettingsRepository.setPlaybackSpeed()` and read `playbackSpeedProvider`; neither goes through `PlaybackService`. The five-value cycle (`kPlaybackSpeedOptions` in `playback_providers.dart`, 0.75x–2x) is shared by both UI entry points so they can never drift out of sync.
- **`ThemeMode` is kept out of the data layer.** `SettingsRepository.watchThemeMode()`/`setThemeMode()` read/write a plain `'light'/'dark'/'system'` string — no repository in `lib/data/repositories/` had ever imported `package:flutter/material.dart` before this, and `PlayerRepeatMode` set the existing precedent for keeping platform/package enums out of the data layer. The string-to-`ThemeMode` parsing lives in the new `lib/data/providers/theme_providers.dart` instead.
