...
- **v1 (build now, phases 1–7):** the shipping product. Complete offline music player with everything listed in "Core capabilities" above.
  ...
  - **Phase 4 is next: Playlists.** Per `DESIGN_MAP.md`: the add-to-playlist bottom sheet (`designs/add_to_playlist/`), the playlist creation flow (`designs/playlist_creation_mini_player/`), and Favorites/Recently Added/Playlists-list/Playlist-detail screens — the last group has no Stitch design folder, so **ask before building** those. The song context menu's "Add to Playlist" action is already stubbed with a snackbar, ready to be wired up. Any new Playlist detail (or other pushed) screen must use `AppScaffold`, per the architectural note above.
